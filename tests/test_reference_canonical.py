...
class CanonicalReferenceTests(unittest.TestCase):
    ...
    def test_exact_grch37_ucsc_hg19_contract_uses_16571_base_mitochondrion(self) -> None:
        contigs = grch37_ucsc_hg19_canonical_25_contigs()

        summary = validate_grch37_ucsc_hg19_canonical_25(contigs)

        self.assertEqual(summary.genome_build, GenomeBuild.GRCH37)
        self.assertEqual(summary.contig_count, 25)
        self.assertEqual(summary.total_reference_bases, 3_095_693_983)
        self.assertEqual(contigs[-1], ("chrM", 16571))

    def test_exact_grch37_ucsc_hg19_rejects_every_dictionary_variant(self) -> None:
        canonical = grch37_ucsc_hg19_canonical_25_contigs()
        variants = (
            canonical[:-1],
            (*canonical, ("chrUn_gl000220", 161802)),
            (*canonical[:-1], ("chrM", 16569)),
        ...
        for contigs in variants:
            with (
                ...
                self.assertRaisesRegex(ValueError, "GRCh37/UCSC hg19 Canonical-25"),
            ):
                validate_grch37_ucsc_hg19_canonical_25(contigs)
    ...
    def test_hg19_run_lock_cannot_be_fabricated_from_native_grch37_lock(self) -> None:
        source_contigs = (
        ...
        source_lock = ReferenceLock(
            reference_id="GRCh37_GENCODE19_TEST",
        ...
        with self.assertRaisesRegex(ValueError, "exact contract-specific FASTA/FAI"):
            reference_lock_for_dictionary_contract(
                source_lock,
                ReferenceDictionaryContract.GRCH37_UCSC_HG19_CANONICAL_25,
            )

    def test_hg19_contract_lock_keeps_its_own_exact_fai_identity(self) -> None:
        contract_lock = ReferenceLock(
            reference_id="GRCh37_GENCODE19_HG19_TEST",
            genome_build=GenomeBuild.GRCH37,
            contigs=[
                ReferenceContig(name=name, length=length)
                for name, length in grch37_ucsc_hg19_canonical_25_contigs()
            ],
            allow_extra_contigs=True,
            source_fai_sha256="d" * 64,
        )
        ...
        self.assertFalse(selected.allow_extra_contigs)
        self.assertEqual(selected.source_fai_sha256, "d" * 64)
        self.assertEqual(selected.contigs, contract_lock.contigs)

    def test_hg19_contract_cannot_be_derived_from_grch38(self) -> None:
        source_lock = ReferenceLock(
            reference_id="GRCh38_TEST",
            genome_build=GenomeBuild.GRCH38,
            contigs=[
                ReferenceContig(name=name, length=length)
                for name, length in grch38_canonical_25_contigs()
            ],
            source_fai_sha256="c" * 64,
        )
        with self.assertRaisesRegex(ValueError, "non-GRCh37"):
            reference_lock_for_dictionary_contract(
                source_lock,
                ReferenceDictionaryContract.GRCH37_UCSC_HG19_CANONICAL_25,
            )
