...
from ontseq_platform.models import PanelBundle, ReferenceDictionaryContract
...
class CommittedPanelSourceTests(unittest.TestCase):
    def test_bundle_declares_both_published_grch38_dictionary_contracts(self) -> None:
        bundle = PanelBundle.model_validate(
            yaml.safe_load((BUNDLE_ROOT / "bundle.yaml").read_text(encoding="utf-8"))
        )
        self.assertEqual(
