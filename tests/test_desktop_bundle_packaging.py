...
def test_desktop_bundle_builds_the_runtime_installer_contract() -> None:
    """The published Windows bundle must contain every file RuntimePackage.VerifyAsync requires."""
    ...
    for marker in required_build_contract:
        assert marker in workflow, f"desktop-ci.yml does not package required runtime input: {marker}"

    required_bundle_checks = (
        '$wheel = "desktop/publish/runtime/ontseq_platform-${{ env.ONTSEQ_VERSION }}-py3-none-any.whl"',
        '$checksums = "desktop/publish/runtime/SHA256SUMS"',
    )
    for marker in required_bundle_checks:
        assert marker in workflow, f"desktop-ci.yml does not verify required bundle input: {marker}"
