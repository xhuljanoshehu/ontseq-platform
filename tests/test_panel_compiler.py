...
from ontseq_platform.models import GenomeBuild, PanelBundle
...
class PanelDerivativeCompilerTests(unittest.TestCase):
    ...
    def test_curated_gene_body_must_overlap_mapped_selection(self) -> None:
        with TemporaryDirectory() as raw:
            root = Path(raw)
            cache = root / "annotations.sqlite"
            _annotation_cache(cache)
            selection = root / "selection.bed"
            selection.write_text(
                "chr22\t100\t200\tBCR\tENSG_BCR\n",
                encoding="utf-8",
            )
            gene_map = root / "target-gene-map.tsv"
            gene_map.write_text(
                "source_target_label\ttarget_label\tdeclared_chromosome\tgene_id\t"
                "native_gene_name\tresolution\n"
                "BCR\tBCR\tchr22\tENSG_BCR\tBCR\tdirect\n",
                encoding="utf-8",
            )
            ...
            with self.assertRaisesRegex(
                ...
                compile_panel_derivatives(
                    selection,
                    cache,
                    root / "analysis_roi.bed",
                    root / "transcripts.tsv",
                    target_gene_map=gene_map,
                )
    ...
    def test_grch37_derivatives_use_curated_gene_id_without_runtime_alias_inference(self) -> None:
        with TemporaryDirectory() as raw:
            root = Path(raw)
            cache = root / "annotations.sqlite"
            ...
            with closing(sqlite3.connect(cache)) as connection:
                connection.execute(
                    "INSERT INTO genes VALUES (?, ?, ?, ?, ?, ?, ?)",
                    ("ENSG_SEPT2", "SEPT2", "chr2", 1_000, 2_000, "+", "protein_coding"),
                )
                connection.execute(
                    """
                    INSERT INTO transcripts VALUES
            ...
            selection = root / "selection.bed"
            selection.write_text(
                "chr2\t500\t2500\tSEPTIN2\tENSG_SEPT2\n",
                encoding="utf-8",
            )
            gene_map = root / "target_gene_map.tsv"
            gene_map.write_text(
                "source_target_label\ttarget_label\tdeclared_chromosome\tgene_id\t"
                "native_gene_name\tresolution\n"
                "SEPTIN2\tSEPTIN2\tchr2\tENSG_SEPT2\tSEPT2\t"
            ...
            summary = compile_panel_derivatives(
                selection,
                cache,
                roi,
                transcripts,
                expected_genome_build=GenomeBuild.GRCH37,
                target_gene_map=gene_map,
            )
            roi_text = roi.read_text(encoding="utf-8")
        ...
        self.assertEqual(summary.resolved_target_count, 1)
        self.assertEqual(summary.unresolved_targets, ())
        self.assertEqual(roi_text, "chr2\t1000\t2000\tSEPTIN2\tENSG_SEPT2\n")
        self.assertIn("SEPTIN2\ttrue", transcript_text)

    def test_target_gene_map_preserves_selection_unmapped_intent(self) -> None:
        with TemporaryDirectory() as raw:
            root = Path(raw)
            cache = root / "annotations.sqlite"
            _annotation_cache(cache, genome_build="GRCh37")
        ...
        self.assertEqual(summary.resolved_target_count, 1)
        self.assertEqual(summary.unresolved_targets, ("SPLIT_REVIEW_REQUIRED",))

    def test_explicit_grch37_compilation_rejects_a_grch38_cache(self) -> None:
        with TemporaryDirectory() as raw:
            root = Path(raw)
            cache = root / "annotations.sqlite"
            _annotation_cache(cache, genome_build="GRCh38")
            selection = root / "selection.bed"
            selection.write_text("chr22\t0\t30000000\tBCR\n")
            with self.assertRaisesRegex(PanelCompilerError, "require a GRCh37"):
                compile_panel_derivatives(
                    selection,
                    cache,
                    root / "roi.bed",
...
class PanelMaterializationTests(unittest.TestCase):
    ...
    def test_publish_candidates_are_adjacent_to_destinations_for_acl_inheritance(self) -> None:
        with TemporaryDirectory() as raw:
            root = Path(raw)
            bundle_root = root / "SYNTHETIC_GRCh38_v1"
            bundle_root.mkdir()
            ...
            destinations = {
                manifest_path,
                bundle_root / "derived" / "analysis_roi.bed",
                bundle_root / "derived" / "transcripts.tsv",
            }
            ...
            with patch("ontseq_platform.panel_compiler.os.replace", side_effect=track_replace):
            ...
            self.assertFalse(any(".panel-compile." in path.name for path in bundle_root.rglob("*")))
