...
def config(api, tmp_path):
    ...
    return api.GATKConfig(
        ...
        tumor=api.Sample("TUMOR", bam, bai, ref.sha256),
        intervals=bed,
    )
    ...
    def __call__(self, argv, cwd, timeout):
        ...
        tool = next(
            (
                ...
                for x in argv
                if x
                in {"Mutect2", "FilterMutectCalls", "GetPileupSummaries", "CalculateContamination"}
            ),
            None,
        )
        if tool == self.fail:
...
def test_plans_both_modes_and_keeps_stats(api, config, tmp_path):
    ...
    normal = api.Sample(
        "NORMAL",
        lock(api, tmp_path / "normal.bam", "synthetic-normal"),
        lock(api, tmp_path / "normal.bam.bai"),
...
@pytest.mark.parametrize(
    ...
    [
        "",
        vcf_text(sample_names=("WRONG",)),
        vcf_text(ad="18"),
        vcf_text(af="nan"),
        vcf_text(af="1.1"),
        vcf_text(ad="-1,2"),
        vcf_text(alt="<DEL>"),
        vcf_text().replace("chr1\t2", "chr1\t0"),
    ],
)
...
def test_header_only_vcf_is_no_call(api, config, tmp_path):
    ...
    result = api.run_gatk(
        config, tmp_path / "run", allow_experimental_ont=True, runner=SyntheticRunner(filtered=text)
    )
    assert result["status"] == "NO_CALL"
...
def resource(api, config, path, technology="not_applicable", assay_id=""):
    ...
    return api.VCFResource(
        locked, lock(api, Path(str(path) + ".tbi")), config.reference.sha256, technology, assay_id
    )
...
def test_resources_and_contamination_path(api, config, tmp_path, paired):
    ...
    if paired:
        normal = api.Sample(
            "NORMAL",
            lock(api, tmp_path / "normal.bam", "normal"),
            lock(api, tmp_path / "normal.bam.bai"),
            config.reference.sha256,
        )
...
def test_allele_specific_filters_are_not_promoted_by_site_pass(api, tmp_path):
    ...
    path.write_text(
        vcf_text(ad="15,3,2", af="0.15,0.10", alt="T,G").replace(
