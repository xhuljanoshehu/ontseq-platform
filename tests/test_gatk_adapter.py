...
def config_dict(tmp_path):
    ...
    return {
        ...
        "assay_id": "synthetic-ont-assay-v1",
        "mode": "tumor_only",
        "reference": {
            "reference_id": "synthetic-ref-v1",
            "genome_build": "synthetic",
            "fasta": ref,
            "fai": locked(tmp_path / "reference.fa.fai", "chrSynthetic\t8\t14\t8\t9\n"),
            "dictionary": locked(
                tmp_path / "reference.dict", "@HD\tVN:1.6\n@SQ\tSN:chrSynthetic\tLN:8\n"
            ),
        },
        "tumor": {
            "sample_id": "SYNTHETIC_T",
            "bam": tumor,
...
def parse(tmp_path, text, **kwargs):
    ...
    path.write_text(text)
    return extract_candidates(path, run_id="synthetic-run-001", sample_id="SYNTHETIC_T", **kwargs)
...
def test_same_bam_digest_rejected(config_dict, tmp_path):
    ...
    with_normal(config_dict, tmp_path)["normal"]["bam"]["sha256"] = config_dict["tumor"]["bam"][
        "sha256"
    ]
    with pytest.raises(ValidationError):
        config(config_dict)
...
def test_multiallelic_fields_are_allele_specific(tmp_path):
    body = (
        "chrSynthetic\t3\t.\tG\tA,T\t.\tPASS\tAS_FilterStatus=SITE|weak_evidence"
...
def test_paired_vcf_sample_order_not_assumed(tmp_path):
    body = "chrSynthetic\t2\t.\tC\tT\t.\tPASS\t.\tGT:AD:AF:DP\t0/0:19,1:0.05:20\t0/1:8,2:0.2:10\n"
    records = parse(
        tmp_path, vcf_text(("SYNTHETIC_N", "SYNTHETIC_T"), body), normal_sample_id="SYNTHETIC_N"
    )
...
def test_gzipped_vcf_supported(tmp_path):
    ...
    assert len(extract_candidates(path, run_id="synthetic-run-001", sample_id="SYNTHETIC_T")) == 1
...
class SyntheticExecutor:
    ...
    def __call__(self, step, workdir, timeout):
        ...
        if step.name == "version":
            out.write_text(
                "The Genome Analysis Toolkit (GATK) v"
            ...
            for output in step.outputs:
                ...
                if output.name.endswith(".vcf.gz"):
                    ...
                    if paired:
                        body = (
                            "chrSynthetic\t2\t.\tC\tT\t.\tPASS\t.\tGT:AD:AF:DP"
...
@pytest.mark.parametrize(
    "failure", ["version", "tumor_validate", "tumor_sample", "mutect2", "filter"]
)
...
def test_empty_valid_vcf_not_negative(config_dict, tmp_path):
    ...
    result = run_mutect2(
        enabled(config_dict), tmp_path / "out", executor=SyntheticExecutor(empty_vcf=True)
    )
    assert result.status == "COMPLETED"
...
def test_native_process_transport_is_shell_free_and_scrubs_java_environment(tmp_path, monkeypatch):
    ...
    result = _native_execute(
        Step("transport", (sys.executable, "-c", program, payload)), tmp_path, 10
    )
    assert result == 0
...
def test_cli_lock_writes_disabled_json_that_can_be_read(config_dict, tmp_path):
    ...
    command = [
        ...
        "synthetic",
        "--reference",
        config_dict["reference"]["fasta"]["path"],
        "--tumor-bam",
        config_dict["tumor"]["bam"]["path"],
        "--tumor-sample",
        "SYNTHETIC_T",
        "--germline",
        config_dict["germline_resource"]["vcf"]["path"],
        "--gatk-jar",
        config_dict["runtime"]["gatk_jar"]["path"],
        "--output",
        str(output),
    ]
    completed = subprocess.run(command, capture_output=True, text=True, env=os.environ.copy())
