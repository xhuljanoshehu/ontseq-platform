...
class BamResolutionTests(unittest.TestCase):
    ...
    def test_explicit_hg19_profile_accepts_exact_25_contig_dictionary(self) -> None:
        hg19 = grch37_ucsc_hg19_canonical_25_contigs()
        with TemporaryDirectory() as raw:
            bam = Path(raw) / "201123_B418-ngmlr8.s.bam"
            bam.write_bytes(b"BAM")
            Path(f"{bam}.bai").write_bytes(b"BAI")

            resolved = resolve_bam_header(
                bam_path=bam,
                header_text=_header(hg19),
                reference_lock=_lock(GenomeBuild.GRCH37, hg19),
                required_build=GenomeBuild.GRCH37,
                dictionary_contract=(ReferenceDictionaryContract.GRCH37_UCSC_HG19_CANONICAL_25),
            )

        self.assertEqual(resolved.genome_build, GenomeBuild.GRCH37)
        self.assertEqual(
            resolved.reference_dictionary_contract,
            ReferenceDictionaryContract.GRCH37_UCSC_HG19_CANONICAL_25,
        )
        self.assertEqual(resolved.contigs, hg19)

    def test_hg19_profile_rejects_native_mitochondrial_length_and_extra_contigs(self) -> None:
        hg19 = grch37_ucsc_hg19_canonical_25_contigs()
        variants = ((*hg19[:-1], ("chrM", 16569)), (*hg19, ("GL000191.1", 106433)))
        for observed in variants:
            with self.subTest(observed=len(observed)), TemporaryDirectory() as raw:
                bam = Path(raw) / "sample.bam"
                bam.write_bytes(b"BAM")
                Path(f"{bam}.bai").write_bytes(b"BAI")
                with self.assertRaisesRegex(ValueError, "exactly match"):
