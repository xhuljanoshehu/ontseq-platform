...
def test_serve_defaults_resolve_every_release_owned_input_outside_the_cwd() -> None:
    ...
    with TemporaryDirectory() as raw:
        ...
        with (
            ...
            patch(
                "ontseq_platform.profile_analysis.__file__",
                str(
                    temporary
                    / "packed/lib/python3.11/site-packages"
                    / "ontseq_platform/profile_analysis.py"
                ),
            ),
...
def test_checkout_uses_its_own_configs_not_an_older_tool_runtime() -> None:
    with TemporaryDirectory() as raw:
        runtime = Path(raw)
        shutil.copytree(CONFIGS, runtime / "share/ontseq/configs")
        with patch("ontseq_platform.profile_analysis.sys.prefix", str(runtime)):
            assert configuration_root() == CONFIGS
...
def test_run_defaults_include_alignment_basecalling_and_release_owned_knowledge() -> None:
    ...
    assert args.methylation_policy == CONFIGS / "methylation" / "modkit.technical.yaml"
...
def test_service_instance_id_is_a_strict_per_launch_nonce() -> None:
    instance_id = "a" * 32
    args = _parser().parse_args(
        [
            "serve",
    ...
    assert args.instance_id == instance_id

    for invalid in ("", "A" * 32, "a" * 31, "../foreign-listener"):
