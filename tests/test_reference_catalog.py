...
class DeterministicReferenceDerivationTests(unittest.TestCase):
    ...
    def test_hg19_contract_fasta_streams_native_nuclear_records_and_replaces_chrm(self) -> None:
        source_bytes = (
            b">chr1 native GRCh37\nACGT\nac\n"
            b">chr2 native GRCh37\nTTT\nGG\n"
            b">chrM native 16569 placeholder\nAAAA\n"
            b">GL000191.1 ignored\nC\n"
        )
        expected_contract = (("chr1", 6), ("chr2", 5), ("chrM", 7))
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            source = root / "native.fa"
            source_fai = root / "native.fa.fai"
            chrm_archive = root / "ucsc.hg19.chrM.fa.gz"
            output = root / "analysis-reference" / "ucsc-hg19-canonical25.fa"
            output_fai = Path(f"{output}.fai")
            source.write_bytes(source_bytes)
            _fasta_index(source, source_fai)
            with (
                chrm_archive.open("wb") as raw_handle,
                gzip.GzipFile(fileobj=raw_handle, mode="wb", mtime=0) as compressed,
            ):
                compressed.write(b">chrM\nACGTNnn\n")

            with patch(
                "ontseq_platform.reference_catalog.grch37_ucsc_hg19_canonical_25_contigs",
            ...
            _fasta_index(output, output_fai)
            validate_fasta_fai_consistency(output, output_fai)
            self.assertEqual(
                [line.split("\t")[:2] for line in output_fai.read_text().splitlines()],
                [["chr1", "6"], ["chr2", "5"], ["chrM", "7"]],
            )

    def test_hg19_contract_fasta_rejects_wrong_publisher_chrm_length(self) -> None:
        expected_contract = (("chr1", 4), ("chrM", 7))
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            source = root / "native.fa"
            source_fai = root / "native.fa.fai"
            chrm_archive = root / "ucsc.hg19.chrM.fa.gz"
            output = root / "ucsc-hg19-canonical25.fa"
            ...
            with (
                ...
                self.assertRaisesRegex(ValueError, "must contain 7 bases, found 6"),
            ):
                _build_grch37_ucsc_hg19_canonical_25_fasta(
                    source,
                    source_fai,
...
class ReferenceBundleInstallerTests(unittest.TestCase):
    ...
    def test_install_prefers_checksum_valid_local_catalog_sources_without_network(self) -> None:
        recipe, contents = _recipe()
        with (
            tempfile.TemporaryDirectory() as local_raw,
            tempfile.TemporaryDirectory() as target_raw,
        ):
            local_root = Path(local_raw)
            for resource in recipe.resources:
                if resource.generated:
                    continue
                source = local_root.joinpath(*resource.path.split("/"))
                source.parent.mkdir(parents=True, exist_ok=True)
                source.write_bytes(contents[resource.resource_id])
            opener = _MemoryOpener({})

            installed = ReferenceBundleInstaller(Path(target_raw), opener=opener).install(
                recipe, local_source_root=local_root
            )

            self.assertTrue(installed.validation.valid)
            self.assertEqual(opener.requests, [])
            self.assertEqual(
                (installed.path / "sources" / "gencode.gtf").read_bytes(),
                contents["gencode"],
            )

    def test_install_does_not_fallback_to_network_for_an_invalid_local_source(self) -> None:
        recipe, contents = _recipe()
        with (
            tempfile.TemporaryDirectory() as local_raw,
            tempfile.TemporaryDirectory() as target_raw,
            ...
            local_root = Path(local_raw)
            genome = local_root / "genome.fa"
            genome.write_bytes(b"tampered-local-source\n")
            opener = _MemoryOpener(contents)

            with self.assertRaisesRegex(ValueError, "local source 'genome'"):
                ReferenceBundleInstaller(Path(target_raw), opener=opener).install(
                    recipe,
    ...
    def test_dns_failure_is_reported_with_resource_url_and_offline_remediation(self) -> None:
        recipe, _ = _recipe()

        def dns_failure(_url: str) -> io.BytesIO:
            raise urllib.error.URLError(socket.gaierror(-3, "Temporary failure in name resolution"))

        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            installer = ReferenceBundleInstaller(root, opener=dns_failure)

            with self.assertRaises(RuntimeError) as raised:
                installer.install(recipe)

            message = str(raised.exception)
            self.assertIn("DNS-Auflösung in WSL fehlgeschlagen", message)
            self.assertIn("'genome'", message)
            self.assertIn("https://fixtures.invalid/genome", message)
            self.assertIn("ontseq references import PATH", message)
            self.assertFalse((root / "references" / recipe.bundle_id).exists())
    ...
    def test_catalog_source_accepts_only_matching_authority_pins_for_generated_files(self) -> None:
        authority, _contents = _recipe()
        authority = _change_resource(
            authority,
            "annotation_cache",
            sha256="a" * 64,
            size_bytes=123,
        ...
        changed = _change_resource(authority, "annotation_cache", sha256="b" * 64)
        with self.assertRaisesRegex(ValueError, "immutable catalog source/generator contract"):
            validate_reference_recipe_contract(
                changed,
                authority,
    ...
    def test_cli_install_passes_the_catalog_bundle_as_preferred_local_source(self) -> None:
        try:
            from ontseq_platform.resource_commands import handle_references_command
        except ModuleNotFoundError as error:
            if error.name != "ontseq_platform.resource_commands":
        ...
        recipe, _ = _recipe()
        installed = MagicMock()
        installed.bundle = recipe
        installed.path = Path("installed") / recipe.bundle_id
        installer = MagicMock()
        installer.install.return_value = installed
        config_root = Path("catalog-config")
        args = argparse.Namespace(
            command="references",
            references_command="install",
        ...
        catalog = MagicMock()
        catalog.get.return_value = recipe
        ...
        with (
            ...
            patch("ontseq_platform.resource_commands._config_root", return_value=config_root),
            patch("ontseq_platform.resource_commands._catalog", return_value=catalog),
            patch("ontseq_platform.resource_commands._bootstrap_if_official"),
            redirect_stdout(io.StringIO()),
        ):
            self.assertTrue(handle_references_command(args))
        ...
        installer.install.assert_called_once_with(
            recipe,
            offline=False,
            local_source_root=config_root / "reference_bundles" / recipe.bundle_id,
        )
