...
import ontseq_platform.resource_bootstrap as resource_bootstrap_module
...
class Grch38ResourceBootstrapTests(unittest.TestCase):
    ...
    def test_repair_atomically_replaces_an_unreadable_regular_profile(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            resource_root = Path(raw).resolve()
            _install_official_id_reference(resource_root)
            bootstrapper = GRCh38ResourceBootstrapper(
            ...
            bootstrapper.activate()
            profile = resource_root / "profiles" / "AML_LCWGS_GRCh38.yaml"
            expected = (CONFIGS / "profiles" / profile.name).read_bytes()
            real_read_bytes = Path.read_bytes

            def deny_destination_bytes(path: Path) -> bytes:
                if path == profile:
                    raise PermissionError(13, "synthetic profile ACL denies reading", str(path))
                return real_read_bytes(path)

            with patch.object(Path, "read_bytes", deny_destination_bytes):
                result = bootstrapper.repair()

            self.assertEqual(result.repaired_paths, (profile,))
            self.assertEqual(profile.read_bytes(), expected)
            self.assertTrue(profile.is_file())

    def test_publish_candidates_are_adjacent_for_file_and_directory_acl_inheritance(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as raw:
            resource_root = Path(raw).resolve()
            transaction = resource_root / ".bootstrap-staging" / "transaction"
            staged_bundle = transaction / "panels" / "SYNTHETIC_PANEL"
            staged_profile = transaction / "profiles" / "SYNTHETIC_PROFILE.yaml"
            staged_bundle.mkdir(parents=True)
            staged_profile.parent.mkdir(parents=True)
            (staged_bundle / "bundle.yaml").write_bytes(b"bundle: staged\n")
            staged_profile.write_bytes(b"profile: staged\n")
            bundle_destination = resource_root / "panels" / "SYNTHETIC_PANEL"
            profile_destination = resource_root / "profiles" / "SYNTHETIC_PROFILE.yaml"
            destinations = {bundle_destination, profile_destination}
            ...
            with patch(
                ...
                activated, repaired, already_active = (
                    GRCh38ResourceBootstrapper._apply_destinations(
                        (
                            (staged_bundle, bundle_destination, True),
                            (staged_profile, profile_destination, False),
                        ),
            ...
            self.assertEqual(activated, [bundle_destination, profile_destination])
            self.assertEqual(repaired, [])
            self.assertEqual(already_active, [])
            self.assertEqual({destination for _source, destination in published}, destinations)
            self.assertEqual(len(published), len(destinations))
            for source, destination in published:
                with self.subTest(destination=destination):
                    self.assertEqual(source.parent, destination.parent)
            self.assertEqual(
                (bundle_destination / "bundle.yaml").read_bytes(),
                b"bundle: staged\n",
            )
            self.assertEqual(profile_destination.read_bytes(), b"profile: staged\n")
            self.assertFalse(any(".publish-" in path.name for path in resource_root.rglob("*")))

    def test_repair_replaces_a_regular_bundle_with_an_unreadable_acl_child(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            resource_root = Path(raw).resolve()
            transaction = resource_root / ".bootstrap-staging" / "transaction"
            staged_bundle = transaction / "panels" / "SYNTHETIC_PANEL"
            destination = resource_root / "panels" / "SYNTHETIC_PANEL"
            staged_bundle.mkdir(parents=True)
            destination.mkdir(parents=True)
            (staged_bundle / "bundle.yaml").write_bytes(b"bundle: repaired\n")
            (destination / "bundle.yaml").write_bytes(b"bundle: unreadable\n")
            real_tree_digest = resource_bootstrap_module._tree_digest

            def deny_destination_tree(path: Path) -> str:
                if path == destination:
                    raise PermissionError(
                        13,
                        "synthetic child ACL denies reading",
                        str(path / "bundle.yaml"),
                    )
                return real_tree_digest(path)

            with patch.object(
                resource_bootstrap_module,
                "_tree_digest",
                side_effect=deny_destination_tree,
            ):
                activated, repaired, already_active = (
                    GRCh38ResourceBootstrapper._apply_destinations(
                        ((staged_bundle, destination, True),),
                        resource_root=resource_root,
                        transaction=transaction,
            ...
            self.assertEqual(already_active, [])
            self.assertEqual(
                (destination / "bundle.yaml").read_bytes(),
                b"bundle: repaired\n",
            )

    def test_activate_keeps_an_unreadable_regular_profile_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            resource_root = Path(raw).resolve()
            _install_official_id_reference(resource_root)
            bootstrapper = GRCh38ResourceBootstrapper(
            ...
            with (
                patch.object(Path, "read_bytes", deny_destination_bytes),
                self.assertRaisesRegex(PermissionError, "synthetic profile ACL denies reading"),
            ):
                bootstrapper.activate()

            self.assertEqual(profile.read_bytes(), expected)

    def test_repair_keeps_an_unreadable_staged_profile_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            resource_root = Path(raw).resolve()
            staged = resource_root / ".bootstrap-staging" / "profile.yaml"
            destination = resource_root / "profiles" / "profile.yaml"
            staged.parent.mkdir(parents=True)
            destination.parent.mkdir(parents=True)
            staged.write_bytes(b"trusted-packaged-profile\n")
            destination.write_bytes(b"existing-profile\n")
            real_read_bytes = Path.read_bytes

            def deny_staged_bytes(path: Path) -> bytes:
                if path == staged:
                    raise PermissionError(13, "synthetic packaged source ACL", str(path))
                return real_read_bytes(path)

            with (
                patch.object(Path, "read_bytes", deny_staged_bytes),
                self.assertRaisesRegex(PermissionError, "synthetic packaged source ACL"),
            ):
            ...
            self.assertEqual(destination.read_bytes(), b"existing-profile\n")
