...
def output_label(target: Target) -> str:
    """Expose unresolved source contradictions in every generated derivative."""
...
def write_bed(path: Path, targets: list[Target], *, panel_version: str) -> None:
    ...
    for target in ordered:
        lines.append(
            f"{target.chromosome}\t{target.start}\t{target.end}\t{output_label(target)}"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_lock(
    ...
    *,
    ...
    document = {
        ...
        "promotion_blockers": [
            "Original fusion_panel_with_buffer.bed has not been obtained and byte-compared.",
            "Coordinate convention is unconfirmed.",
            "IGH_REVIEW_REQUIRED label/coordinate contradiction is unresolved.",
        ],
    }
