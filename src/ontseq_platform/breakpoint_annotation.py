"""Typed, build-locked breakpoint annotation over a compiled reference cache.
...
from .annotation_cache import require_annotation_cache_build
...
class BreakpointAnnotationError(ValueError):
    """The breakpoint or annotation cache violates its build-locked contract."""
...
def _require_build_cache(connection: sqlite3.Connection, *, expected_build: str) -> None:
    try:
        ...
        raise BreakpointAnnotationError(str(exc)) from exc
    ...
    *,
    ...
    try:
        with closing(
            ...
            _require_build_cache(connection, expected_build=expected_build)
    ...
    *,
    ...
    for event in events:
        ...
        context_flags = [
            ...
            for hit in annotation.contexts
            # The shipped Umap intervals mark uniquely mappable sequence, not artifacts.
            # Preserve the hit in breakpoint.contexts; absent rows do not prove poor mapping.
            if hit.resource_type != "mappability"
        ]
        orientation = next(
            (
