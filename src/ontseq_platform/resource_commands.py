...
def add_references_parser(
    ...
    references = subparsers.add_parser(
        "references", help="Install and validate manifest-locked GRCh37/GRCh38 resources"
    )
    ...
    install = commands.add_parser("install", help="Install a pinned native reference recipe")
...
def _authority_catalog_for_import(source: Path) -> ReferenceCatalog | None:
    ...
    bundle_id = _declared_import_bundle_id(source)
    if bundle_id is None:
        return None
    catalog = _packaged_authority_catalog()
    try:
        catalog.get(bundle_id)
    except KeyError:
        return None
    return catalog
...
def _bootstrap_if_official(
    ...
    if bundle_id == REFERENCE_BUNDLE_ID:
        bootstrapper_type = GRCh38ResourceBootstrapper
    elif bundle_id == GRCH37_REFERENCE_BUNDLE_ID:
        bootstrapper_type = GRCh37ResourceBootstrapper
    else:
        return
    bootstrapper = bootstrapper_type(
        args.resource_root,
    ...
    panel_detail = (
        ...
        if result.panel_summary is not None
        else "; panel resources: not required"
    )
    print(
...
def handle_references_command(args: argparse.Namespace) -> bool:
    ...
    if command == "status":
        ...
        registries = [
            ResourceRegistry(args.resource_root, active_build=build) for build in GenomeBuild
        ]
        profile_status = [
            item
            for registry in registries
            for item in _profile_status(registry, verify_checksums=False)
        ]
        ready_profiles = [
                            ...
                            for item in sorted(
                                {
                                    ...
                                    if diagnostic.code != "inactive_build"
                                },
    ...
    if command == "validate":
        ...
        if args.bundle_id is None:
            validation_profile_status = [
                ...
                for item in _profile_status(
                    ResourceRegistry(args.resource_root, active_build=build),
    ...
    if command in {"install", "repair"}:
        if args.bundle_id in {REFERENCE_BUNDLE_ID, GRCH37_REFERENCE_BUNDLE_ID}:
            catalog_root = _packaged_config_root() / "reference_bundles"
            recipe = _packaged_authority_catalog().get(args.bundle_id)
        else:
            catalog_root = _config_root(args) / "reference_bundles"
            try:
        ...
        installed = (
            installer.install(
                ...
                local_source_root=catalog_root / recipe.bundle_id,
            )
            if command == "install"
            else installer.repair(
                recipe,
                offline=args.offline,
                local_source_root=catalog_root / recipe.bundle_id,
            )
