...
def _resource(
    path: Path, name: str, reference_digest: str, assay_id: str | None = None
) -> VcfResource:
    path = path.expanduser().resolve(strict=True)
    return VcfResource(
        name=name,
        vcf=_lock(path),
        index=_lock(Path(str(path) + ".tbi")),
        reference_sha256=reference_digest,
        kind="ont_pon" if assay_id else "population",
        assay_id=assay_id,
    )
...
def lock_config(
    *,
    ...
    pon: Path | None = None,
    contamination_sites: Path | None = None,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Experimental local ONT/Mutect2 adapter. Research Use Only. No clinical release."
        )
    )
    actions = parser.add_subparsers(dest="action", required=True)
    locking = actions.add_parser(
        "lock", help="Hash local inputs and create a disabled configuration"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            json.dumps({"status": "FAILED", "error": str(exc), "clinically_reportable": False}),
            file=sys.stderr,
        )
        return 1
