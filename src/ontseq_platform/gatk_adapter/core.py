...
from .contracts import LockedFile, Mutect2Config, Mutect2Result
...
def locked_inputs(config: Mutect2Config) -> dict[str, LockedFile]:
    ...
    files = {
        'reference.fasta': reference.fasta, 'reference.fai': reference.fai,
        'reference.dictionary': reference.dictionary, 'gatk.jar': config.runtime.gatk_jar,
        'tumor.bam': config.tumor.bam, 'tumor.index': config.tumor.bai,
    }
    if config.normal:
        files.update({'normal.bam': config.normal.bam, 'normal.index': config.normal.bai})
    for label, resource in (
        ('germline', config.germline_resource), ('pon', config.panel_of_normals),
...
def preflight(config: Mutect2Config) -> dict[str, str]:
    ...
    for sample in samples:
        ...
        alternatives = valid | {
            Path(str(sample.bam.path) + '.csi'), sample.bam.path.with_suffix('.csi')
        }
    ...
    for resource in (config.germline_resource, config.panel_of_normals,
                     config.contamination_sites):
        if resource:
...
def build_plan(config: Mutect2Config, workdir: Path) -> tuple[Step, ...]:
    ...
    prefix = (runtime.java_executable, f'-Xmx{runtime.heap_gb}g', '-jar',
    ...
    for role, sample in (('tumor', config.tumor), ('normal', config.normal)):
        ...
        steps += [
            Step(f'{role}_validate', (*prefix, 'ValidateSamFile', '--INPUT',
                 str(sample.bam.path), '--REFERENCE_SEQUENCE', reference, '--MODE', 'SUMMARY',
                 '--INDEX_VALIDATION_STRINGENCY', 'EXHAUSTIVE', '--OUTPUT', str(validation)),
                 (validation,)),
            Step(f'{role}_sample', (*prefix, 'GetSampleName', '-I', str(sample.bam.path),
                 '-O', str(sample_file)), (sample_file,)),
        ]
    ...
    if config.estimate_contamination:
        ...
        for role, sample in (('tumor', config.tumor), ('normal', config.normal)):
            ...
            args = [*prefix, 'GetPileupSummaries', '-R', reference, '-I', str(sample.bam.path),
                    '-V', sites, '-L', sites]
            if config.intervals:
                args += ['-L', str(config.intervals.path), '--interval-set-rule', 'INTERSECTION']
            args += ['-O', str(pileup)]
            steps.append(Step(f'{role}_pileup', tuple(args), (pileup,)))
        args = [*prefix, 'CalculateContamination', '-I', str(workdir / 'tumor.pileups.table'),
                '-O', str(contamination), '--tumor-segmentation', str(segmentation)]
        if config.normal:
    ...
    args = [*prefix, 'FilterMutectCalls', '-R', reference, '-V', str(raw),
            '--stats', str(stats), '--filtering-stats', str(filter_stats), '-O', str(filtered)]
    if config.estimate_contamination:
        args += ['--contamination-table', str(contamination),
                 '--tumor-segmentation', str(segmentation)]
    steps.append(Step('filter', tuple(args),
                      (filtered, Path(str(filtered) + '.tbi'), filter_stats)))
    return tuple(steps)
...
def _native_execute(step: Step, workdir: Path, timeout: int) -> int:
    ...
    stdout_fd = os.open(workdir / f'{step.name}.stdout.log',
                        os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    stderr_fd = os.open(workdir / f'{step.name}.stderr.log',
                        os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    with os.fdopen(stdout_fd, 'wb') as stdout, os.fdopen(stderr_fd, 'wb') as stderr:
        options = {'umask': 0o077} if os.name == 'posix' else {}
        completed = subprocess.run(
            list(step.argv), cwd=workdir, env=environment, stdin=subprocess.DEVNULL,
            stdout=stdout, stderr=stderr, shell=False, timeout=timeout, check=False, **options,
        )
...
def _check_contamination(path: Path, sample_id: str) -> None:
    lines = [line for line in _bounded_text(path).splitlines()
...
def run_mutect2(
    config: Mutect2Config, output_dir: Path, *, executor: Executor | None = None,
) -> Mutect2Result:
    ...
    """
    base = {
        'run_id': config.run_id, 'sample_id': config.tumor.sample_id, 'mode': config.mode,
        'config_sha256': config_sha256(config),
    }
    if not config.enabled or not config.allow_experimental_ont:
        return Mutect2Result(**base, status='NOT_RUN', messages=(
            'Explicit enabled=true and allow_experimental_ont=true are both required.',
        ))
    ...
    try:
        ...
        _private_json(output_dir / 'commands.json', [
        ...
        for step in plan:
            ...
            if step.name in {'version', 'java_version'}:
                text = (_bounded_text(output_dir / f'{step.name}.stdout.log') + '\n'
                        + _bounded_text(output_dir / f'{step.name}.stderr.log'))
                if step.name == 'version':
                    versions = re.findall(r'(?:GATK\)?\s*v?|Version[:\s]+)(\d+\.\d+\.\d+\.\d+)',
        ...
        candidates = extract_candidates(
            output_dir / 'filtered.vcf.gz', run_id=config.run_id,
    ...
    result = Mutect2Result(
        **base, status=status, messages=tuple(messages), candidates=candidates,
        input_sha256s=inputs, output_sha256s=outputs, step_exit_codes=exit_codes,
