"""Deterministic, build-locked SQLite transcript and cytoband annotation.
...
_GENCODE19_RELEASE = re.compile(r"^(?:GENCODE\s*)?(?:release\s*)?v?19$", re.IGNORECASE)


def require_annotation_cache_build(
    connection: sqlite3.Connection, *, expected_build: str = "GRCh38"
) -> None:
    """Require the consumer's build, including native GRCh37 annotation provenance."""

    if expected_build not in {"GRCh37", "GRCh38"}:
        raise ValueError(f"unsupported annotation genome build: {expected_build!r}")
    metadata = dict(connection.execute("SELECT key, value FROM metadata").fetchall())
    observed = metadata.get("genome_build")
    if observed != expected_build:
        raise ValueError(
            f"annotation cache requires {expected_build}, found {observed!r}; "
            "cross-build annotation is prohibited"
        )
    if expected_build == "GRCh37" and (
        metadata.get("assembly_name") != "GRCh37.p13"
        or metadata.get("mane_status") != "not_available_native_grch37"
        or metadata.get("mane_release") != "not_applicable"
        or "mane_sha256" in metadata
        or not _GENCODE19_RELEASE.fullmatch(metadata.get("gencode_release", ""))
    ):
        raise ValueError("GRCh37 cache requires native GENCODE 19 / GRCh37.p13 without MANE")
...
def compile_annotation_cache(
    ...
    *,
    ...
    build = cache_metadata.get("genome_build")
    if build == "GRCh37":
        if mane_gff3 is not None:
            raise ValueError(
                "MANE-backed compilation accepts only genome_build=GRCh38; "
                "native GRCh37 requires GENCODE 19 without MANE"
            )
        if not _GENCODE19_RELEASE.fullmatch(cache_metadata.get("gencode_release", "")):
            raise ValueError("native GRCh37 annotation requires an explicit GENCODE 19 release")
        if cache_metadata.get("assembly_name", "GRCh37.p13") != "GRCh37.p13":
            raise ValueError("native GRCh37 annotation requires assembly_name=GRCh37.p13")
        if "mane_sha256" in cache_metadata:
            raise ValueError("native GRCh37 annotation must not declare a MANE checksum")
        cache_metadata.update(
            assembly_name="GRCh37.p13",
            mane_status="not_available_native_grch37",
            mane_release="not_applicable",
        )
    elif build == "GRCh38":
        if mane_gff3 is None:
            raise ValueError("GRCh38 annotation requires the pinned MANE source")
    else:
        raise ValueError("annotation compilation requires genome_build=GRCh37 or GRCh38")

    sources = [("GENCODE", gencode_gtf), ("cytoband", cytobands)]
    if mane_gff3 is not None:
        sources.append(("MANE", mane_gff3))
    ...
    elif any(item.mane_status is not None for item in transcripts):
        raise ValueError("native GENCODE 19 input may not claim MANE transcript tags")
...
def validate_annotation_cache(
    path: Path, *, expected_build: str = "GRCh38"
) -> AnnotationCacheSummary:
    ...
    try:
        ...
        if (
            ...
            and connection.execute(
                "SELECT 1 FROM transcripts WHERE mane_status IS NOT NULL "
                "OR mane_refseq_id IS NOT NULL LIMIT 1"
            ).fetchone()
            is not None
        ):
            raise ValueError("native GRCh37 cache may not claim MANE transcript matches")
...
class AnnotationCache:
    ...
    def __init__(self, path: Path, *, expected_build: str = "GRCh38") -> None:
        self.path = path
