...
_UCSC_HG19_CHRM_FASTA_ARCHIVE_ROLE = "ucsc_hg19_chrm_fasta_archive"
_DERIVED_FASTA_LINE_BASES = 1024 * 1024
...
def _load_reference_bundle(path: Path) -> ReferenceBundle:
    ...
    if bundle.genome_build == GenomeBuild.GRCH37 and any(
        resource.role == "mane_gff3" for resource in bundle.resources
    ):
        raise ValueError(
            "MANE-backed reference recipes support only GRCh38; "
            "native GRCh37.p13 recipes must use GENCODE 19 without MANE"
        )
    return bundle
    ...
    if candidate_ids != authority_ids:
        ...
        for candidate_resource, authority_resource in zip(
            ...
            if (
                ...
                authority_resource.sha256 is None
                and authority_resource.generated
            ...
            if (
                ...
                authority_resource.size_bytes is None
                and authority_resource.generated
                and not allow_materialized_generated_pins
...
def _validate_resource(
    ...
    *,
    ...
    if resource.role == "annotation_cache" and verify_checksum:
        try:
            validate_annotation_cache(path, expected_build=expected_build)
    ...
    *,
    ...
    resources = tuple(
        _validate_resource(
            ...
            expected_build=bundle.genome_build.value,
        )
    ...
    contract_fastas = [
        item for item in bundle.resources if item.role == GRCH37_UCSC_HG19_CANONICAL_25_FASTA_ROLE
    ]
    contract_indexes = [
        item for item in bundle.resources if item.role == GRCH37_UCSC_HG19_CANONICAL_25_FAI_ROLE
    ]
    if contract_fastas or contract_indexes:
        try:
            if bundle.genome_build != GenomeBuild.GRCH37:
                raise ValueError("UCSC hg19 contract FASTA is valid only in a GRCh37 bundle")
            if len(contract_fastas) != 1 or len(contract_indexes) != 1:
                raise ValueError(
                    "UCSC hg19 contract requires exactly one analysis FASTA and one FASTA index"
                )
            contract_fasta = _resource_path(bundle_path, contract_fastas[0])
            contract_fai = _resource_path(bundle_path, contract_indexes[0])
            if contract_fasta.is_file() and contract_fai.is_file():
                if verify_checksums:
                    validate_fasta_fai_consistency(contract_fasta, contract_fai)
                ...
                validate_grch37_ucsc_hg19_canonical_25(
                    (item.name, item.length) for item in contract_lock.contigs
                )
        except (OSError, UnicodeError, ValueError) as exc:
            errors.append(f"UCSC hg19 analysis FASTA validation failed: {exc}")
    return BundleValidationReport(
...
def _copy_fai_sequence(
    source_handle: BinaryIO,
    output_handle: BinaryIO,
    record: tuple[str, int, int, int, int],
) -> None:
    """Copy one indexed FASTA sequence without retaining a chromosome in memory.

    Source line endings are validated against the FAI. Output is deterministically wrapped at
    one MiB so later FASTA validation cannot read a whole chromosome into memory, even when the
    publisher FASTA stores one chromosome per line, without incurring millions of small writes.
    """

    name, length, offset, line_bases, line_width = record
    source_handle.seek(offset)
    remaining = length
    output_column = 0
    while remaining:
        bases_this_line = min(line_bases, remaining)
        unread = bases_this_line
        while unread:
            chunk = source_handle.read(min(unread, 1024 * 1024))
            if not chunk or b"\n" in chunk or b"\r" in chunk:
                raise ValueError(f"source FASTA sequence {name!r} disagrees with its FAI")
            chunk_offset = 0
            while chunk_offset < len(chunk):
                available = _DERIVED_FASTA_LINE_BASES - output_column
                piece = chunk[chunk_offset : chunk_offset + available]
                output_handle.write(piece)
                chunk_offset += len(piece)
                output_column += len(piece)
                if output_column == _DERIVED_FASTA_LINE_BASES:
                    output_handle.write(b"\n")
                    output_column = 0
            unread -= len(chunk)
        remaining -= bases_this_line
        if remaining:
            separator = source_handle.read(line_width - line_bases)
            if separator not in {b"\n", b"\r\n"}:
                raise ValueError(f"source FASTA wrapping for {name!r} disagrees with its FAI")
    if output_column:
        output_handle.write(b"\n")


def _read_single_fasta_archive(source: Path, *, expected_name: str) -> bytes:
    with gzip.open(source, "rb") as handle:
        header = handle.readline().strip()
        if header.split(maxsplit=1)[0] != f">{expected_name}".encode("ascii"):
            raise ValueError(f"FASTA archive must contain the single record {expected_name!r}")
        sequence = b"".join(line.strip() for line in handle if line.strip())
    if not sequence or any(base not in b"ACGTNacgtn" for base in sequence):
        raise ValueError(f"FASTA archive {expected_name!r} contains invalid sequence bytes")
    return sequence


def _build_grch37_ucsc_hg19_canonical_25_fasta(
    source_fasta: Path,
    source_fai: Path,
    chrm_archive: Path,
    output: Path,
) -> None:
    """Materialize the exact hg19 dictionary using native nuclear GRCh37 plus UCSC chrM.

    GRC patch releases do not alter the existing primary chromosome sequences.  The nuclear
    records therefore come from the already checksum-pinned GENCODE19 GRCh37.p13 FASTA, while
    the original UCSC hg19 16,571-base NC_001807 record is supplied by its own publisher file.
    No coordinate conversion or run-time sequence substitution is performed.
    """

    records = _read_fasta_index_records(source_fai)
    by_name = {record[0]: record for record in records}
    if len(by_name) != len(records):
        raise ValueError("source GRCh37 FASTA index contains duplicate sequence names")
    expected = grch37_ucsc_hg19_canonical_25_contigs()
    nuclear = expected[:-1]
    for name, length in nuclear:
        if name not in by_name or by_name[name][1] != length:
            raise ValueError(
                f"source GRCh37 FASTA lacks the exact nuclear sequence {name}:{length}"
            )
    chrm = _read_single_fasta_archive(chrm_archive, expected_name="chrM")
    if len(chrm) != expected[-1][1]:
        raise ValueError(f"UCSC hg19 chrM must contain {expected[-1][1]} bases, found {len(chrm)}")
    output.parent.mkdir(parents=True, exist_ok=True)
    with source_fasta.open("rb") as source_handle, output.open("wb") as output_handle:
        for name, _length in nuclear:
            output_handle.write(f">{name}\n".encode("ascii"))
            _copy_fai_sequence(source_handle, output_handle, by_name[name])
        output_handle.write(b">chrM\n")
        output_handle.write(chrm)
        output_handle.write(b"\n")


_GRCH37_NUCLEAR_CONTEXT_CONTIGS = frozenset(
    [f"chr{number}" for number in range(1, 23)] + ["chrX", "chrY"]
)


def _normalize_ucsc_context_table(
    source: Path,
    output: Path,
    *,
    role: str,
    ...
    Native GRCh37.p13 uses only the shared nuclear chromosomes from UCSC hg19 context.
    UCSC hg19 chrM differs from GRCh37.p13, and alternate-contig aliases are not inferred.
    """
    ...
    with (
        ...
        if genome_build == GenomeBuild.GRCH37:
            output_handle.write(
                "# GRCh37.p13 context scope: chr1-22,chrX,chrY; "
                "excludes hg19 chrM/alternate contigs\n"
            )
        for line_number, raw in enumerate(source_handle, start=1):
            ...
            if (
                genome_build == GenomeBuild.GRCH37
                and chromosome not in _GRCH37_NUCLEAR_CONTEXT_CONTIGS
            ):
                continue
...
def _normalize_grch37_blacklist(source: Path, output: Path) -> None:
    """Keep only the hg19 blacklist coordinates shared with native GRCh37.p13."""
    ...
    with (
        ...
        output_handle.write(
            "# GRCh37.p13 context scope: chr1-22,chrX,chrY; excludes hg19 chrM/alternate contigs\n"
        )
        for line_number, raw in enumerate(source_handle, start=1):
            ...
            fields = raw.rstrip("\r\n").split("\t")
            if len(fields) < 3:
                raise ValueError(f"blacklist source line {line_number} requires BED3 coordinates")
            try:
                start, end = int(fields[1]), int(fields[2])
            except ValueError as exc:
                raise ValueError(
                    f"blacklist source line {line_number} has non-integer coordinates"
                ) from exc
            if not fields[0] or start < 0 or end <= start:
                raise ValueError(f"blacklist source line {line_number} has an invalid interval")
            if fields[0] not in _GRCH37_NUCLEAR_CONTEXT_CONTIGS:
                continue
            label = fields[3] if len(fields) > 3 and fields[3] else "technical_blacklist"
            output_handle.write(f"{fields[0]}\t{start}\t{end}\t{label}\n")
...
class ReferenceBundleInstaller:
    ...
    def _download(self, resource: ResourceFile, target: Path, *, offline: bool) -> None:
        ...
        try:
            ...
            is_dns_error = isinstance(reason, socket.gaierror) or any(
            ...
            raise RuntimeError(
                "DNS-Auflösung in WSL fehlgeschlagen beim Abruf der gepinnten Quelle "
                f"{resource.resource_id!r} von {resource.source_url}. Prüfe Internetzugang "
                "und DNS in WSL. Es war keine verifizierte lokale Quelldatei verfügbar; für "
                "eine Offline-Installation die vollständigen checksum-validierten Quellen "
                "neben der Bundle-Recipe bereitstellen oder ein aktiviertes Bundle mit "
                "'ontseq references import PATH' importieren."
            ) from exc
    ...
    def _generate(
        ...
        if resource.role == "genome_fasta":
        ...
        elif resource.role == GRCH37_UCSC_HG19_CANONICAL_25_FASTA_ROLE:
            dependencies = [bundle.resource(resource_id) for resource_id in resource.derived_from]
            by_role = {item.role: _resource_path(bundle_path, item) for item in dependencies}
            required_roles = {
                "genome_fasta",
                "fasta_index",
                _UCSC_HG19_CHRM_FASTA_ARCHIVE_ROLE,
            }
            if len(dependencies) != 3 or set(by_role) != required_roles:
                raise ValueError(
                    "derived UCSC hg19 analysis FASTA requires the native genome FASTA, "
                    "its FAI and the pinned UCSC chrM archive"
                )
            _build_grch37_ucsc_hg19_canonical_25_fasta(
                by_role["genome_fasta"],
                by_role["fasta_index"],
                by_role[_UCSC_HG19_CHRM_FASTA_ARCHIVE_ROLE],
                output,
            )
        elif resource.role == "fasta_index":
            _, fasta_path = self._role_path(bundle, bundle_path, "genome_fasta")
            _fasta_index(fasta_path, output)
        elif resource.role == GRCH37_UCSC_HG19_CANONICAL_25_FAI_ROLE:
            if len(resource.derived_from) != 1:
                raise ValueError("derived UCSC hg19 analysis FAI requires exactly one FASTA")
            fasta_resource = bundle.resource(resource.derived_from[0])
            if fasta_resource.role != GRCH37_UCSC_HG19_CANONICAL_25_FASTA_ROLE:
                raise ValueError("UCSC hg19 analysis FAI must derive from its contract FASTA")
            _fasta_index(_resource_path(bundle_path, fasta_resource), output)
            ...
            elif any(item.role == "mane_gff3" for item in bundle.resources):
                raise ValueError("native GRCh37.p13 recipes must not include MANE resources")
            ...
            _normalize_ucsc_context_table(
                source, output, role=resource.role, genome_build=bundle.genome_build
            )
        elif resource.role == "blacklist" and bundle.genome_build == GenomeBuild.GRCH37:
            if len(resource.derived_from) != 1:
                raise ValueError("derived GRCh37 blacklist requires exactly one pinned BED archive")
            source_resource = bundle.resource(resource.derived_from[0])
            if source_resource.role != "blacklist_archive":
                raise ValueError("derived GRCh37 blacklist source must have role blacklist_archive")
            _normalize_grch37_blacklist(_resource_path(bundle_path, source_resource), output)
        else:
        ...
        *,
        ...
        for resource in recipe.resources:
            ...
            if only_resource_ids is None or resource.resource_id in only_resource_ids:
                ...
                try:
                    source = (
                        _resource_path(local_source_root, resource)
                        if local_source_root is not None
                        else None
                    )
                    # A recipe directory may optionally carry any or all publisher byte streams.
                    # Prefer those checksum-pinned local bytes. Missing files may use the network
                    # for install/repair, while an existing but invalid local file is never hidden
                    # by a download fallback.
                    if source is not None and path_lexists(source):
                        if not source.is_file() or is_link_like(source):
                            raise FileNotFoundError(f"pinned local source is unsafe: {source}")
                        if (
                            resource.size_bytes is not None
                            and (observed_size := source.stat().st_size) != resource.size_bytes
                        ):
                            raise ValueError(
                                f"local source {resource.resource_id!r}: expected "
                                f"{resource.size_bytes} bytes, found {observed_size}"
                            )
                        if resource.sha256 is None:
                            raise ValueError(
                                f"local source {resource.resource_id!r} has no pinned SHA256"
                            )
                        observed_hash = sha256_file(source)
                        if observed_hash != resource.sha256:
                            raise ValueError(
                                f"local source {resource.resource_id!r}: expected SHA256 "
