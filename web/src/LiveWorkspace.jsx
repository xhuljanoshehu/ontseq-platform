...
import { artifactFilename, createLiveApi, isTerminalJob, requestedProfile, resolveAvailableProfile, watchRun, workspaceHomeHref } from "./liveApi.js";
...
export function LiveWorkspace() {
  ...
  const [marlinReadiness, setMarlinReadiness] = useState(null);
  useEffect(() => {
    setMarlinReadiness(null);
    if (!bam || api.disabledReason) return undefined;
    const genomeBuild = profile.match(/GRCh(?:37|38)/)?.[0];
    if (!genomeBuild) return undefined;
    const controller = new AbortController();
    api.marlinReadiness(genomeBuild, controller.signal)
      .then(value => { if (!controller.signal.aborted) setMarlinReadiness(value); })
      .catch(error => { if (!controller.signal.aborted) setMarlinReadiness({ready:null,reason:error.message}); });
    return () => controller.abort();
  }, [api, bam?.posix, bam?.selection_id, profile]);
  ...
  async function download(kind) {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = artifactFilename(job, kind);
    ...
    <main className="live-canvas">
      ...
      <section className="live-section" id="live-input"><div className="live-section-title"><span className="live-step-number">1</span><div><h2>BAM auswählen</h2><p className="live-subtle">Wählen Sie eine BAM-Datei aus dem lokalen Dateisystem.</p></div></div>
        <form onSubmit={startRun}>
          ...
          {bam && <Notice><strong>MARLIN · Forschungsmodus</strong><br />{marlinReadiness ? `${marlinReadiness.ready === true ? "Konfiguriert; vollständige Ressourcenprüfung steht noch aus" : marlinReadiness.ready === false ? "Nicht verfügbar" : "Verfügbarkeit nicht festgestellt"}: ${marlinReadiness.reason}` : "Verfügbarkeit für den gewählten Referenzbuild wird geprüft."} Bei gewählter Methylierung wird MARLIN automatisch angefordert. Fehlende Voraussetzungen werden im Lauf als NOT_RUN dokumentiert. UNVALIDATED_RESEARCH · keine klinische Validierung.</Notice>}
