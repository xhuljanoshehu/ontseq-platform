...
### CNV-Benchmark-Subsystem

Getrennt von der Laufzeit-Lane liegt ein Benchmark-Subsystem, das **nie an einer
Patientenprobe läuft**. Es beantwortet die Frage, die vor jeder Freigabe steht: wie gut ist
ein Caller, worauf gemessen, und wo kann er überhaupt keine Aussage treffen.

Gemessen wird basenweise über eine exakte Bruchpunkt-Partition des Genoms, nicht über
Segment-Überlappungen — das Ergebnis hängt damit nicht davon ab, wie eine der beiden Seiten
segmentiert hat. Was nicht bewertbar ist, wird nicht stillschweigend als richtig oder falsch
verbucht, sondern mit einem Grund aus einem geschlossenen Vokabular ausgeschlossen und
basenweise bilanziert. Jede Kennzahl trägt so einen prüfbaren Nenner.

```bash
ontseq cnv-demo-benchmark --output-dir results/cnv-demo
```

Der Demolauf simuliert Read-Depth über Blastanteil- und Coverage-Strata, ruft sie mit einem
transparenten Baseline-Caller, bewertet gegen einen deklarierten Truth-Set über eine
explizite Beobachtbarkeitsmaske und schätzt eine Nachweisgrenze. Er braucht kein externes
Werkzeug, kein Referenzgenom und keine genomischen Daten.

Ein gut gemessener Caller ist damit noch nicht befördert: nichts in diesem Subsystem macht
eine Lane klinisch berichtsfähig. Details in
[`docs/CNV_BENCHMARKING.md`](docs/CNV_BENCHMARKING.md).
...
- [CNV benchmarking](docs/CNV_BENCHMARKING.md)
