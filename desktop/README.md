...
> **Current engineering build: Desktop/Core v0.3.5. Research Use Only, unsigned, not clinically validated.**
...
4. Configure the exact Reference-Lock that matches the BAM (GRCh37 or GRCh38). v0.3.5 requires complete canonical chromosomes 1–22, X and Y for the selected named assembly and revalidates the stored lock before each run.
...
10. The application checks WSL, the v0.3.5 backend capabilities, input/index, configured reference, required runtime resources, output location and Adaptive Sampling BED when applicable.
...
### Reference and resume integrity in v0.3.5
...
See `desktop.settings.example.json` for the available keys. First-run setup writes the user-local settings automatically. A runtime installed by the v0.3.5 Desktop is stored under `~/.local/share/ontseq/runtime-v0.3.5` in the selected WSL distribution.
...
## Deliberate v0.3.5 limitation: cancellation
