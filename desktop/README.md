...
> **Current engineering build: Desktop/Core v0.8.2. Research Use Only, unsigned, not clinically validated.**
...
1. Keep the complete package together. The runtime directory contains a pinned base tool
   archive, the Core v0.8.2 wheel, and their SHA256SUMS. Setup verifies both inputs, unpacks
   the tools into a new `runtime-v0.8.2-<unique-id>` prefix, then installs the new Core
   offline and verifies its version. It does not replace the existing runtime directory.
   Fresh settings use `~/.local/share/ontseq/resources-v0.8.2` for resources. The release
...
```json
{
  "resourceRootWsl": "~/.local/share/ontseq/resources-v0.8.2",
